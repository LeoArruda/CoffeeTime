...
exports.resize = async (req, res, next) => {
    ...
    // once written the photo to our filesystem, keep going@
    next();
}
...
exports.updateStore = async (req, res) =>{
    ...
    // redirect them the store and tell it worked
    res.redirect(`/stores/${store._id}/edit`);
};

exports.getStoreBySlug = async (req, res, next) => {
    //const store = await (await Store.findOne({ slug: req.params.slug })).populated('author');
    const store = await (await Store.findOne({ slug: req.params.slug }));
